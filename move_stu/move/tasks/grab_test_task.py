...
def _with_hand_closure(q: torch.Tensor, dof_names: list[str], close_amount: float = 1.0) -> torch.Tensor:
    closed = q.clone()
    for index, name in enumerate(dof_names):
        if name.startswith("xhand_left_") or name.startswith("xhand_right_"):
            closed[index] = _finger_close_value(name, close_amount)
    return closed


def _apply_timeline_hand_closure(timeline, dof_names: list[str]):
    closed_timeline = []
    for phase, root_pose, q, center, theta in timeline:
        if phase.startswith("move:") or phase.startswith("place_handoff:") or phase.startswith("place:"):
            q = _with_hand_closure(q, dof_names, 1.0)
        closed_timeline.append((phase, root_pose, q, center, theta))
    return closed_timeline
...
def main() -> None:
    ...
    parser.add_argument("--contact-only", action="store_true", help="Disable kinematic attach; box must move only through contact dynamics.")
    ...
    print("  box_mode=contact_only" if args.contact_only else "  box_mode=contact_then_kinematic_attach")
    ...
    try:
        while viewer is None or not gym.query_viewer_has_closed(viewer):
            ...
            if (not args.contact_only) and not attached and not released and frame >= args.attach_after_pick_frames:
                attached = True
