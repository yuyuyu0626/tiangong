...
脚本内容：
1. 垛型为 1.0 x 1.0 x 1.6 m，分成 3 x 5 x 4，共 60 个箱子；
2. 第 1、2 层使用直接放置、释放、收手和推入策略；
3. 第 3、4 层使用waist-arm 策略；
...
模式说明：
    默认模式        跑完整 60 箱。
    --headless      无 viewer 运行。
    --fast          加速 viewer 播放，并降低渲染频率。
...
from move_pre.grab_test import (  # noqa: E402
...
from move_pre.mobile_ik import MoveLiftIK  # noqa: E402
...
from move_pre.tasks.grab_test_task import (  # noqa: E402
...
def _build_waist_stack_namespace() -> SimpleNamespace:
    ...
    def _stack_stand_off_for_cell(cell: StackCell, base_stand_off: float) -> float:
        ...
        if _uses_fourth_layer_cross_corner(cell):
            return max(float(base_stand_off), FOURTH_LAYER_CROSS_CORNER_STAND_OFF)
        ...
        if _uses_fourth_layer_outer_corner(cell):
            return max(float(base_stand_off), FOURTH_LAYER_OUTER_CORNER_STAND_OFF)
    ...
    def _stack_y_release_clearance_for_cell(cell: StackCell) -> float:
        if _uses_fourth_layer_cross_corner(cell):
            return FIRST_LAYER_Y_NEIGHBOR_RELEASE_CLEARANCE + STACK_CROSS_CENTER_RELEASE_CLEARANCE
        if (
    ...
    def _build_stack_box_timeline(
        ...
        carry_source_center = (
            source_center[0],
            source_center[1],
            max(source_center[2] + STACK_PICK_LIFT_Z, target_center[2] + DIRECT_PREPLACE_CLEARANCE),
        )
...
SECOND_LAYER_INNER_ROW_Y_RELEASE_CLEARANCE = 0.010
SECOND_LAYER_INNER_ROW_X_HAND_CLEARANCE = 0.040
FIRST_LAYER_INNER_EDGE_HAND_CLEARANCE = 0.100
...
def _make_grip_keyframe(
    ...
    from move_pre.utils import CartesianKeyframe
...
def _stand_off_for_cell(cell: StackCell, base_stand_off: float) -> float:
    if _cell_layer_sequence(cell) == 1:
        return base_stand_off
    if cell.layer == 1 and _cell_layer_sequence(cell) in {2, 3}:
    ...
    if cell.layer == 1 and _uses_first_layer_y_row_strategy(cell) and cell.iy in {0, GRID_Y - 1}:
        return max(base_stand_off, SECOND_LAYER_Y_EXTENSION_STAND_OFF)
...
def _inner_edge_push_world_delta(cell: StackCell) -> tuple[float, float]:
    ...
    y_step = 0.0
    if side == "-Y":
        y_step = FIRST_LAYER_INNER_EDGE_RELEASE_CLEARANCE
    elif side == "+Y":
        y_step = -FIRST_LAYER_INNER_EDGE_RELEASE_CLEARANCE
...
def _run_box_timeline(
    ...
    for local_frame, (phase, root_pose, q, box_center, box_theta) in enumerate(timeline):
        ...
        if (
            ...
            and _uses_inner_edge_push_strategy(plan.cell)
...
def main() -> None:
    # 命令行只保留完整 60 箱模式和仿真加速相关开关；分层调试请使用 task1_2.py。
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  python -m move.tasks.task1\n"
            "  python -m move.tasks.task1 --headless\n"
            "  python -m move.tasks.task1 --fast --viewer-render-every 8\n"
        ),
    ...
    plans = [build_direct_box_plan(cell, DIRECT_STAND_OFF) for cell in native_cells]
    for plan in plans:
    ...
    timelines = [build_timeline(plan) for plan in plans]
    ...
    robot, boxes = _create_static_scene(gym, sim, env, robot_asset, box_sequences, proxy_layers=0)
    ...
    try:
        ...
        if keep_running:
            ...
            for plan, box, box_index in zip(waist_plans, waist_boxes, waist_box_indices):
                ...
                global_frame, keep_running = waist_stack._run_stack_box_timeline(
                    gym,
                    sim,
                    env,
                    robot,
